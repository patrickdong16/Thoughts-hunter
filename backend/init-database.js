...
async function initDatabase() {
    ...
    try {
        ...
        // 11. YouTube 互动数据字段迁移
        // ===============================================
        await pool.query(`ALTER TABLE radar_items ADD COLUMN IF NOT EXISTS video_id VARCHAR(20)`);
        await pool.query(`ALTER TABLE radar_items ADD COLUMN IF NOT EXISTS yt_view_count INTEGER`);
        await pool.query(`ALTER TABLE radar_items ADD COLUMN IF NOT EXISTS yt_like_count INTEGER`);
        await pool.query(`ALTER TABLE radar_items ADD COLUMN IF NOT EXISTS yt_comment_count INTEGER`);
        await pool.query(`ALTER TABLE radar_items ADD COLUMN IF NOT EXISTS yt_updated_at TIMESTAMP`);
