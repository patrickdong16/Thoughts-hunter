...
### 作者归属规范 (v5.1.34+)
- **author_name**: 必须是真实人名，禁止使用机构名或来源名（如 "Google News"）
- **source**: 必须包含日期和发布场合（格式：`YYYY年M月D日 · 媒体/会议名称`）
...
## 📱 微信公众号发布（2026-01-29 新增）

### 核心服务

| 文件 | 功能 |
|------|------|
| `wechat-publisher.js` | 微信 API 交互（access_token/草稿/发布） |
| `routes/wechat.js` | 微信相关 API 端点 |
...
| `WECHAT_DEFAULT_THUMB_ID` | 默认封面图 media_id |

### IP 白名单

微信 API 要求 IP 白名单，Railway 动态 IP 需手动更新。

---

## 🔍 两阶段质检架构（2026-01-29 新增）

### 设计原则

内容质量检查和推送配额控制分离，各自独立执行。
...
|--------|------|
| 内容长度 | ≥300 字符（生成时≥700） |
| 必填字段 | freq, stance, title, author_name, content |
| 频段有效 | T1/T2/T3/P1/P2/Φ1/Φ3/H1/H2/R2/F2 |
| 立场有效 | yes/no 或 A/B |
| 作者格式 | ≥2字符，无特殊标记 |

### 阶段二：配额质检（radar_items → 推送）

| 渠道 | 每日配额 | 每频段配额 | 推送时间 |
|------|----------|------------|----------|
| 微信 | 5 篇 | 1 篇 | 7:00-22:00 |
| APP | 10 篇 | 2 篇 | 7:00-22:00 |

### 核心函数

| 函数 | 阶段 | 用途 |
|------|------|------|
| `validateItem()` | 阶段一 | 单条内容质检 |
| `validatePublishQuota()` | 阶段二 | 推送配额验证 |

---

## 🎯 张力问题设计 v2.0（2026-01-29 新增）

### 设计目标

- **统计可汇总**：用户投票收敛到时代级问题趋势
- **问题个性化**：每篇文章 `tension_q` 贴合具体内容
- **立场标准化**：固定「是/否」二元标签
...
| 频段 | 核心问题 | 是 | 否 |
|------|---------|-----|-----|
| T1 | AI是否正在加剧社会分层? | 正在加剧 | 趋向普惠 |
| T2 | 技术是否正在绕开制度约束? | 正在绕开 | 可被规制 |
| T3 | 技术发展是否已脱离人类掌控? | 正在失控 | 仍可引导 |
| P1 | 民主制度是否正在失效? | 正在失效 | 仍然有效 |
| P2 | 权力是否正在脱离公众视野? | 正在隐蔽化 | 仍可监督 |
| Φ1 | 自由与安全是否必须二选一? | 必须取舍 | 可以兼得 |
| Φ3 | 算法优化是否正在侵蚀自由意志? | 正在侵蚀 | 只是工具 |
| H1 | 当前文明是否正在走向衰退? | 正在衰退 | 只是调整 |
| H2 | 历史是否正在重复衰落模式? | 正在重复 | 时代不同 |
| R2 | 科技是否正在成为新宗教? | 正在替代 | 只是工具 |
| F2 | 金融是否正在加剧社会撕裂? | 正在加剧 | 可以修复 |

### AI 生成规则

1. `tension_q`：个性化问题（以"是否"开头，贴合文章内容）
2. `stance`：必须是 `"yes"` 或 `"no"`（小写）
3. `tension_a/b`：从 `BAND_DEFINITIONS` 获取固定标签

---

*本文件仅对思想雷达项目生效*
*最后更新：2026-01-29（微信发布 + 两阶段质检 + 张力问题 v2.0）*
