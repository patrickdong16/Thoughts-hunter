...
将思想雷达内容同步发布到微信公众号，支持在微信生态内直接阅读。
...
| 端点 | 方法 | 用途 |
|------|------|------|
| `/api/wechat/status` | GET | 检查微信配置状态 |
| `/api/wechat/sync-today` | POST | 同步今日内容到草稿箱 |
| `/api/wechat/drafts` | GET | 获取草稿列表 |
| `/api/wechat/publish/:mediaId` | POST | 发布指定草稿 |
| `/api/wechat/materials` | GET | 获取素材列表（封面图） |
...
│  ─────────────────────────────────────────────              │
│  ✓ 内容长度 ≥300 字符                                        │
│  ✓ 必填字段完整 (freq, stance, title, author_name, content)  │
│  ✓ 频段有效、立场有效 (yes/no 或 A/B)                         │
│  ✓ 作者姓名格式正确                                          │
│  ✓ 来源信息存在                                              │
└─────────────────────────────────────────────────────────────┘
                            │
...
| 文件 | 功能 |
|------|------|
| `content-validator.js` | 两阶段质检逻辑 |
| `draft-manager.js` | 草稿审批（集成内容质检） |
| `wechat-publisher.js` | 微信推送（集成配额质检） |
...
1. **统计可汇总**：用户投票可以收敛到时代级问题的趋势监测
2. **问题个性化**：每篇文章的 `tension_q` 贴合具体内容
3. **立场标准化**：使用固定的「是/否」二元标签
...
```json
{
    "tension_q": "GPT-5的发布是否会进一步加剧AI技术的精英垄断?",
    "stance": "yes",
    "tension_a": "是，正在加剧",
    "tension_b": "否，趋向普惠"
}
...
| 日期 | 变更 |
|------|------|
| 2026-01-29 | v4.3 - ① 微信公众号发布功能：内容自动同步到微信草稿箱 ② 张力问题优化：11个时代级核心问题 + 是/否二元立场 ③ 两阶段质检架构：内容质检(drafts→items) + 配额质检(items→推送) |
